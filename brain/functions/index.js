...
export const telegramWebhook = onRequest(
    ...
    async (req, res) => {
        // Telegram espera 200 OK inmediato para no reintentar
...
Si no necesitás herramientas, respondé directamente.${historyText}`;
        ...
        if (geminiResponse.functionCalls?.length > 0) {
            ...
            // Crear PC_Job en Firestore (La Garra lo detecta y ejecuta)
            const jobRef = await db.collection('PC_Jobs').add({
                ...
                expiresAt: Timestamp.fromMillis(Date.now() + 120_000) // 2 min
            });
